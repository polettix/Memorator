...
wrapper around the constructor L</new>, calls L</initialize> as well as
...
initialize an instance attaching to the L</minion> and ensuring that the
...
constructor. This does I<not> install tasks in L<Minion>, which needs
L</initialize>. See L</create> for a constructor that I<does what you
mean>.

The recognized keys in C<%args> correspond to accessors
L</alert_callback>, L</minion> and L</name>.
